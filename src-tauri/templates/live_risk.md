...
{% else %}
{% for p in positions %}
### {{ p.inst_id }}（{{ p.pos_side | side }} {{ p.leverage | num(0) }}× {{ p.margin_mode | margin }}）
...
- **{{ m.inst_id }}**：趋势 {{ m.trend | na("不可判定") }}｜波动 {{ m.volatility | na("不可判定") }}｜拥挤度 {{ m.crowding }}
  ...
  - RSI14 {{ m.rsi14 | na("不可得") | num(2) }}｜ATR {{ m.atr_pct | na("不可得") | pct }}｜基差 {{ m.basis_pct | na("不可得") | pct }}
{% endfor %}
...
- 保证金率：{{ account.margin_ratio | na("不可得") | num(2) }}
{% else %}
