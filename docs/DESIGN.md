...
│  │  └─ settings/              # 设置：隐私等级 / 界面主题 / 网络代理 / 缓存 / 诊断
│  └─ lib/
...
| `theme_get` / `theme_set` | `{theme}` | `Theme` | 全局界面主题（`system` / `light` / `dark`），落 `settings.theme` |
...
日间 / 夜间 / 跟随系统，全局设置（`settings.theme` + 设置页 + `theme_get` / `theme_set`），默认**跟随系统**：大多数人不会专门去调主题，而系统偏好已经表达了意图。

**实现方式是这套配色的关键**：不改组件，而是**按层级翻转 CSS 变量**。

Tailwind v4 的工具类编译出来是变量引用（可直接在构建产物里核对：`.bg-neutral-900{background-color:var(--color-neutral-900)}`），所以在 `html[data-theme="light"]` 作用域下重定义这些变量，全站用法一起换色。翻转规则是 `n` 与其对位互换（50↔950、100↔900、…、500 不动），取值全部取自 Tailwind 自己的 `theme.css` 的对位色号，不是手调近似色。

> **要记住的代价（ADR 26）**：这么翻之后，色号表示的是**相对层级**而不是绝对明度。看到 `bg-neutral-900` 要理解成「一级表面」而不是「深灰」。换来的是不动 875 处类名——本项目没有视觉回归测试，逐处改才是真正的风险来源。

两处例外必须单独处理：

- **CodeMirror（模板编辑器）**：它自己的配色是内联样式，拿不到那套变量，所以 `TemplateEditor` 里显式定义了深浅两套；切换用 `Compartment` 重配置而不是重建编辑器（否则正在编辑的人会丢焦点与撤销历史）。
- **首屏着色**：设置存在 SQLite 里、只能异步读回来，所以 `main.tsx` 先按系统偏好猜一帧 → 跟随系统的用户没有闪烁；显式选了另一档的用户会看到一帧反色。这是「不让本地再存一份主题状态」换来的一致性——两份状态迟早会不一致。

`color-scheme` 跟着一起切（滚动条、原生表单控件才会跟着变）。
...
| `SettingsPage.test.tsx`（隐私卡片 / 主题卡片） | 隐私：三档都能选、点击写回 `privacy_set`、并按 `privacy_get` 的值标出当前档（不写死默认）。主题：三档都渲染、点击写回 `theme_set`、**「跟随系统」时写出实际生效的档位**（否则用户看不出现在到底是哪一档） |
| `useTheme.test.ts` | `resolveTheme` 的三条路径（显式日间 / 显式夜间 / 跟随系统），以及默认值必须是 `system`（与 Rust `Theme::default()` 一致，不一致会让首屏闪一下） |
| `AppGate.test.tsx`（主题） | 顶层真的把主题写到了 `<html data-theme>`——漏掉这次顶层调用的话，设置页点了没反应，且非常难查 |
...
| **26** | **界面主题用「按层级翻转 CSS 变量」，不给每个组件写第二套类名** | Tailwind v4 的工具类编译成 `var(--color-neutral-900)`，于是在 `html[data-theme=light]` 下重定义这些变量，全站一起换色——不改组件、也就不会漏改。本项目没有视觉回归测试，「逐处改 875 个类名」才是真风险 | 逐类加 `dark:` 变体（漏一处就是一处错色，且没人会发现）、语义化 token（更干净，但要逐处改 875 个类名并逐个目视核对——代价与收益在这一轮不成比例） |
